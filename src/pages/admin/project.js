...
import FormProject from '../../components/form/project_form';
  ...
  const handleDeleteSubmit = async (designsId) => {
    if (window.confirm("តើអ្នកពិតជាចង់លុបអត្ថបទនេះមែនទេ?")) {
      try {
        ...
        const response = await api.delete(`/projects/${designsId}`);

        if (response.status === 200 || response.status === 204) {
          setProjects((prevProjects) => prevProjects.filter((project) => project.id !== designsId));
                ...
                <div className='adnvul-row'>
                  <div className='adnvulrw-bx'>
                    ...
                    <div className='adnvulrw-con'>
                      <blockquote>
                        <h2>មានចំនួន {projects.length}</h2> 
